<template>
  <div class="article-detail page-content">
    <div class="content">
      <h1>{{ useStore.ArticleDetail.article_title }}</h1>
      <div class="markdown-body" v-highlight v-html="useStore.ArticleDetail.article_content"></div>
    </div>
    <ArtBackToTop />
  </div>
</template>
...
<script setup lang="ts">
  ...
  //   初始化pinia
  const useStore = useArticlesStore()

  onMounted(() => {
    useCommon().scrollToTop()
    articleId.value = Number(router.query.id)
    console.log('跳转的id:', articleId.value)
    useStore.getArticleDetail(articleId.value)
  })
  // 监听 route.query.id 变化，每次变化都执行
  watch(
    () => router.query.id,
    (newId) => {
      if (!newId) return
